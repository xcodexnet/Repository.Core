...
namespace CBLibrary.Repository.Extensions
{
    ...
    public static class StringExtensions
    {
        ...
        /// <returns></returns>
        public static bool EqualsWithIgnoreCase(this string source, string target)
        {
            if (null == source)
            {
                return target == null;
        ...
        #region AES
        private const string _key = "2ckf8iEFZCreX/MRwVCIOQu1uV8n(K6bUmlty6E+sYE6On0mzcR%qu3ZHFIU7v01FafBMV0+k7P)vanPu3JxAB^kmw==";
        ...
        /// <returns></returns>
        public static string AESEncrypt(this string original, string key = null)
        {
            try
            {
                //1.初始化key
                ...
                //2.将待加密字符串转换成字节数组
                byte[] decryptedBytes = Encoding.UTF8.GetBytes(original);

                AesCryptoServiceProvider aesCryptoService = new AesCryptoServiceProvider();
                //3.设置密钥
                byte[] keyBytes = GetLegalKey(aesCryptoService, key);
                ...
                Byte[] encryptedBytes = cTransform.TransformFinalBlock(decryptedBytes, 0, decryptedBytes.Length);
        ...
        /// <param name="key"></param>
        /// <returns></returns>
        public static string AESDecrypt(this string encrypted, string key = null)
        {
            try
            {
                //1.初始化key
                ...
                Byte[] decryptedBytes = cTransform.TransformFinalBlock(encryptedBytes, 0, encryptedBytes.Length);
        ...
        private static byte[] GetLegalKey(SymmetricAlgorithm symmetricAlgorithm, string key)
        {
            //当在派生类中重写时,生成用于该算法的随机密钥
            symmetricAlgorithm.GenerateKey();
            //获取或设置对称算法的密钥
            var byteTemp = symmetricAlgorithm.Key;
            var keyLength = byteTemp.Length;

            key = key.Length > keyLength ? key.Substring(0, keyLength) : key.PadRight(keyLength, ' ');
